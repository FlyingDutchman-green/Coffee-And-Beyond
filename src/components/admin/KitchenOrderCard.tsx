...
export function KitchenOrderCard({
  ...
  const getStatusBadge = (status: OrderStatus) => {
    switch (status) {
      case "NEW":
        return (
          <span className="text-xs font-medium py-0.5 px-2 rounded-sm bg-[#F7F7F5] border border-[#E7E7E3] text-[#1E1E1C] flex items-center gap-1">
            <Clock className="w-3 h-3 text-[#1E1E1C]" />
            <span>New Order</span>
          </span>
        );
      case "CONFIRMED":
        return (
          <span className="text-xs font-medium py-0.5 px-2 rounded-sm bg-[#FDFBF7] border border-[#E2D9C8] text-[#7A5E28] flex items-center gap-1">
      ...
      case "PREPARING":
        return (
          <span className="text-xs font-medium py-0.5 px-2 rounded-sm bg-[#F4F7FA] border border-[#D2DCE5] text-[#2B4C6F] flex items-center gap-1">
      ...
      case "READY":
        return (
          <span className="text-xs font-medium py-0.5 px-2 rounded-sm bg-[#F5F8F3] border border-[#D3DEC8] text-[#3B5E2B] flex items-center gap-1">
      ...
      case "COMPLETED":
        return (
          <span className="text-xs font-medium py-0.5 px-2 rounded-sm bg-[#FAFAFA] border border-[#E7E7E3] text-[#777772] flex items-center gap-1">
      ...
      case "CANCELLED":
        return (
          <span className="text-xs font-medium py-0.5 px-2 rounded-sm bg-[#FDF6F5] border border-[#ECCEC9] text-[#8C3426] flex items-center gap-1">
  ...
  // Determine single-click action configuration based on business rules
  ...
  return (
    <article className="bg-canvas-primary border border-border-subtle rounded-lg p-4 sm:p-5 flex flex-col justify-between gap-4 shadow-2xs hover:border-[#D0D0CA] transition-colors">
      {/* Header: Table, Order ID, Timestamp & Status Badge */}
      <div className="space-y-2 border-b border-border-subtle pb-3.5">
        <div className="flex items-start justify-between gap-2">
          <div>
            <div className="flex items-center gap-2">
              <h3 className="text-xl font-bold tracking-tight text-[#1E1E1C] font-mono">
                TABLE {order.tableId}
              </h3>
            ...
            >
              <div className="flex items-start justify-between gap-2">
                <div className="flex items-start gap-2.5 flex-1 min-w-0">
                  <span className="bg-[#1E1E1C] text-white px-2 py-0.5 rounded text-xs font-bold font-mono shrink-0 shadow-2xs">
                    {item.quantity}x
                  </span>
                  <div>
        ...
        {action && (
          <div className="space-y-2">
            <button
              ...
              className="w-full h-11 bg-[#1E1E1C] text-white font-semibold text-xs sm:text-sm rounded-md hover:bg-[#3A3A37] transition-colors flex items-center justify-center gap-2 shadow-xs cursor-pointer"
            >
            ...
            {order.status !== "COMPLETED" && (
              <button
                ...
                className="w-full py-1 text-[11px] font-medium text-text-muted hover:text-[#8C3426] transition-colors text-center cursor-pointer"
              >
                Cancel Order
              </button>
            )}
