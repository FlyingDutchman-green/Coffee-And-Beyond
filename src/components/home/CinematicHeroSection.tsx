...
export function CinematicHeroSection() {
  ...
  const [resolvedVideoUrl, setResolvedVideoUrl] = useState<string>(
    hero.videoUrl || DEFAULT_SETTINGS.hero.videoUrl
  );
  ...
  const refreshVideo = useCallback(async () => {
    try {
      ...
      const resolved = await resolveMediaUrl(
        hero.videoUrl,
        DEFAULT_SETTINGS.hero.videoUrl
      );
      setResolvedVideoUrl(resolved || DEFAULT_SETTINGS.hero.videoUrl);
    } catch {
      setResolvedVideoUrl(DEFAULT_SETTINGS.hero.videoUrl);
    }
  }, [hero.videoUrl]);
      ...
      <section className="relative w-full min-h-[560px] sm:min-h-[640px] lg:min-h-[720px] flex items-center justify-center overflow-hidden bg-[#121211] border-b border-border-subtle select-none">
        ...
        <div className="relative z-10 w-full max-w-[1200px] mx-auto px-4 sm:px-6 lg:px-8 py-20 sm:py-24 lg:py-32 flex flex-col justify-between">
          <div className="max-w-3xl space-y-6 sm:space-y-8">
            {/* Brand Vision Badge + Watch Showreel Action */}
            <div className="flex flex-wrap items-center gap-3">
              ...
              <button
                ...
                className="inline-flex items-center gap-1.5 px-3.5 py-1.5 rounded-full bg-black/40 hover:bg-black/60 border border-white/20 text-white text-xs font-semibold backdrop-blur-md transition-all cursor-pointer shadow-sm"
              >
